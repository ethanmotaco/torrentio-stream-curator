...
export function createApp(): express.Express {
  ...
  // strict routing keeps /configure and /configure/ as distinct paths so the
  // trailing-slash redirect below doesn't loop.
  ...
  app.use((req, res, next) => {
    ...
    // Beamup sits behind Cloudflare. Two CF behaviors break inline JS:
    //   1. Auto-minification strips backslashes from regex literals.
    //   2. Edge cache serves stale HTML even after redeploy.
    // no-transform forbids the rewrite; no-store/no-cache forces revalidation
    // on every request so the next deploy is immediately visible.
  ...
  // Cloudflare in front of Beamup aggressively caches /configure for 4h
  // ignoring origin Cache-Control. Force a 301 to /configure/ so users
  // bypass the stale cache key entirely.
  app.get("/configure", (_req, res) => { res.redirect(301, "/configure/"); });
