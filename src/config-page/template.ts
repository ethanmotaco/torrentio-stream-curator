...
// ===== Build install URL =====
function b64url(s) { return btoa(unescape(encodeURIComponent(s))).replace(new RegExp("\\\\+", "g"), "-").replace(new RegExp("/", "g"), "_").replace(new RegExp("=+$"), ""); }
...
// Cache-bust query: CF caches /stats.json at the edge and ignores our
// no-store header on already-cached entries. Unique query per page load
// forces a fresh origin fetch.
...
function hydrateFromUrl() {
  const m = window.location.pathname.match(new RegExp("^/([A-Za-z0-9_-]{40,})/configure/?$"));
  if (!m) return false;
